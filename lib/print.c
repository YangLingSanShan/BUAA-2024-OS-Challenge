...
void vprintfmt(fmt_callback_t out, void *data, const char *fmt, va_list ap) {
	...
	for (;;) {
		/* scan for the next '%' *	 e.g	/*10c%-3d10*/
		...
		if(*fmt == '-'){
			...
			ladjust = 0;		//if "d" or "ld"
			padc = ' ';		//now still is "d" or "ld"
		}
		/* get width */
		...
		if(*fmt == 'l'){
			...
			long_flag = 0;		//if "d10", now still is "d10"
		}
		neg_flag = 0;
		switch (*fmt) {
		case 'b':
		...
		case 'D':
			...
			if(num < 0) {
				neg_flag = 1;
				num *= -1;		
			//'Num' is the third argument in function 'print_num', but it is an unsigned number.
			//Therefore, it should mulitply '-1', and '-' will be appended in the following function. 
			}
			print_num(out, data, num, 10, neg_flag, width, ladjust, padc, 0);
			//line above refer to unsigned int 'u', difference is that 'd' has sign and here we should use neg_flag. 
			break;
