...
int readelf(const void *binary, size_t size) {
	...
	sh_table = binary + ehdr->e_shoff;	//binary + shoff
	sh_entry_count = ehdr->e_shnum;		//number
	sh_entry_size = ehdr->e_shentsize;	//size
	...
	for (int i = 0; i < sh_entry_count; i++) {
		...
		shdr = (Elf32_Shdr *)(i*sh_entry_size + sh_table);
		addr = shdr->sh_addr;
		printf("%d:0x%x\n", i, addr);
	}
