...
int remove(const char *path, int allowRemoveDir) {
	// Call fsipc_remove.
...
int getEnvid(void) {
	return syscall_getenvid();
}

int getStatus(u_int envid) {
//	printf("%d\n", envid);
	return syscall_getStatus(envid);
}

void setStatus(u_int envid, u_int status) {
	int status_id = getStatus(envid);
	if(status_id != 1) {
		printf("fg: (0x%08x) not running\n", envid);
	} else {
		syscall_set_env_status(envid, status);	
	}
//	printf("%d\n",r);
}

void changeEnv(u_int envid, char*s) {
	int status_id = getStatus(envid);
	if(status_id != 1) {
		printf("fg: (0x%08x) not running\n", envid);
	} else {
