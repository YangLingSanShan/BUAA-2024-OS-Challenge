...
int sys_getStatus(u_int envid) {
	struct Env *e;
	try(envid2env(envid, &e, 0));	
	return e->env_status;
}

void sys_changeEnv(u_int envid) {
	//struct Env *e;
	//try(envid2env(envid, &e, 0));
	//curenv = e;	
	
	while(curenv->env_id != envid) {
		sys_yield();
	}
}

void sys_set_res(int res) {
	curenv->env_res = res;
}

int sys_get_res(u_int envid) {
	struct Env *e;
	int r;
...
void *syscall_table[MAX_SYSNO] = {
    ...
    [SYS_changeEnv] = sys_changeEnv,
    [SYS_set_res] = sys_set_res,
    [SYS_get_res] = sys_get_res,
};
